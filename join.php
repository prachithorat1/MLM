...
if(isset($_GET['join_user'])){
	...
	if($pin!='' && $email!='' && $mobile!='' && $address!='' && $account!='' && $under_userid!='' && $side!=''){
		...
		if(pin_check($pin)){
			...
			if(email_check($email)){
				...
				if(userid_check($under_userid)) {
					//Under userid is ok
					if(side_check($under_userid,$side)){
						//Side check
						$flag=1;
						$query = "INSERT INTO users (pin, email, mobile, address, account, under_userid, side) 
                              VALUES ('$pin', '$email', '$mobile', '$address', '$account', '$under_userid', '$side')";
					}
					else{
	...
	if($flag==1){
		...
		$query = mysqli_query($con,"update pin_list set status='open' where pin='$pin'");
	...
	echo '<script>console.log("Flag is NOT set. Possible pin issue.");</script>';
}
?><!--/join user-->
<?php 
//functions
